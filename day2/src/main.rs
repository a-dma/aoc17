// const INPUT: &[&[u32]] = &[&[5, 1, 9, 5], &[7, 5, 3], &[2, 4, 6, 8]];
const INPUT: &[&[u32]] =
    &[&[86, 440, 233, 83, 393, 420, 228, 491, 159, 13, 110, 135, 97, 238, 92, 396],
      ...
      &[4620, 2358, 2195, 4312, 168, 1606, 4050, 102, 2502, 138, 135, 4175, 1477, 2277, 2226,
        1286],
...
fn problem2(input: &[&[u32]]) -> u32 {
    ...
    for line in input {
        for i in 0..line.len() {
            let head = line[i];
            match line.iter().skip(i + 1).find(|&x| x % head == 0 || head % x == 0) {
                Some(x) => {
                    if *x > head {
                        acc += x / head;
                    } else {
...
fn main() {
    println!("checksum: {} {}", problem1(INPUT), problem2(INPUT));
}
